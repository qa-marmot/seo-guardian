...
export function checkDescriptionLength(
  ...
  if (length < min) {
    return {
      ...
      expected,
      selector,
      message: `<meta name="description"> is too short: ${length} chars (min: ${min}). Content: "${trimmed}"`,
      context: input.context,
      url: input.url,
      duration: Date.now() - start,
      helpUrl: 'https://developers.google.com/search/docs/appearance/snippet',
  ...
  if (length > max) {
    return {
      ...
      expected,
      selector,
      message: `<meta name="description"> is too long: ${length} chars (max: ${max}). Content: "${trimmed}"`,
      context: input.context,
      url: input.url,
      duration: Date.now() - start,
      helpUrl: 'https://developers.google.com/search/docs/appearance/snippet',
