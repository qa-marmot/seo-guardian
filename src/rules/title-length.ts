...
export function checkTitleLength(
  ...
  if (!title) {
    return {
      ...
      expected,
      selector: 'title',
      message: `<title> tag is missing or empty. Expected length between ${min} and ${max} characters.\nFix: Add a <title> tag in <head> with a descriptive phrase that includes your primary keyword.`,
  ...
  if (length < min) {
    return {
      ...
      expected,
      selector: 'title',
      message: `<title> is too short: ${length} chars (min: ${min}, max: ${max}). Current value: "${title}"\nFix: Expand the title to at least ${min} characters. Aim for a descriptive phrase that includes your primary keyword.`,
      context: input.context,
      url: input.url,
      duration: Date.now() - start,
      helpUrl: 'https://developers.google.com/search/docs/appearance/title-link',
  ...
  if (length > max) {
    return {
      ...
      expected,
      selector: 'title',
      message: `<title> is too long: ${length} chars (max: ${max}, min: ${min}). Current value: "${title}"\nFix: Shorten the title to ${max} characters or fewer. Remove filler words and keep the core keyword phrase.`,
      context: input.context,
      url: input.url,
      duration: Date.now() - start,
      helpUrl: 'https://developers.google.com/search/docs/appearance/title-link',
