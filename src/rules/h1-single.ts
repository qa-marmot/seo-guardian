...
export function checkH1Single(input: RuleInput): H1Result {
  ...
  if (count === 0) {
    return {
      ...
      expected: { count: 1 },
      selector: 'h1',
      message: 'No <h1> element found. Each page should have exactly one <h1>.\nFix: Add a single <h1> tag that describes the main topic of the page. It should be unique and include your primary keyword.',
  ...
  if (count > 1) {
    return {
      ...
      expected: { count: 1 },
      selector: 'h1',
      message: `Found ${count} <h1> elements, but exactly 1 is required.\nActual values: ${texts.map((t) => `"${t}"`).join(', ')}\nFix: Keep only one <h1> per page. Demote the extra ones to <h2> or lower.`,
      context: input.context,
      url: input.url,
      duration: Date.now() - start,
      helpUrl: 'https://developers.google.com/search/docs/appearance/title-link',
